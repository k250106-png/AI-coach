...
function extractFulfillmentText(response: any): string {
  ...
  const messages = Array.isArray(response?.queryResult?.fulfillmentMessages)
  ...
  return fallback || "I'm sorry, I couldn't generate a response right now.";
}
  ...
  const [result] = await dialogflowClient.detectIntent({
    ...
    queryInput: {
      text: {
        ...
        languageCode: payload.languageCode || env.dialogflowLanguageCode,
      },
    },
  });
